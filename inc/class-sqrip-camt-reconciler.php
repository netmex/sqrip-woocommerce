...
class Sqrip_Camt_Reconciler
{
            ...
            if ($slip_count > 1) {
                for ($index = 1; $index <= $slip_count; $index++) {
                    ...
                    $slips[] = array(
                        'index'        => $index,
                        'reference'    => $reference,
                        // Falls back to the order total only when the per-slip amount is
                        // missing, which happens on invoices generated before 1.11.
    ...
    private function categorise_slip($order, $slip, array $payments)
    {
        ...
        // Invoices from before 1.11 can be missing their per-slip amount. Without an
        // expected amount there is nothing to verify, so this stays manual rather than
        // being confirmed on the strength of the reference alone.
