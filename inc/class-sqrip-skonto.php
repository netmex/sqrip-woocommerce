...
class Sqrip_Skonto
{
    ...
    /**
     * Void the discounted invoice once the ordinary one was paid, or the other way
     * round. Whichever the customer used, the other must stop being payable.
     *
     * Follows the convention the plugin already uses elsewhere: the reference and the
     * file are marked 'deleted', and the PDF leaves the media library.
     *
     * @param WC_Order $order
     * @param string   $keep 'skonto' to void the ordinary invoice, otherwise the
     *                       discounted one is voided.
     * @return bool Whether anything was voided.
     */
    public static function void_alternative($order, $keep)
    {
        if (!is_a($order, 'WC_Order')) {
            return false;
        }

        $prefix = ($keep === 'skonto') ? 'sqrip_' : 'sqrip_skonto_';
        $reference_key = ($keep === 'skonto') ? 'sqrip_reference_id' : 'sqrip_skonto_reference_id';

        $reference = sqrip_get_order_meta_value($order, $reference_key);

        if (!$reference || $reference === 'deleted') {
            return false;
        }

        $attachment_id = (int) sqrip_get_order_meta_value(
            $order,
            ($keep === 'skonto') ? 'sqrip_qr_pdf_attachment_id' : 'sqrip_skonto_qr_pdf_attachment_id'
        );
        ...
        $order->update_meta_data($reference_key, 'deleted');
        $order->update_meta_data($prefix . 'pdf_file_url', 'deleted');
        $order->update_meta_data($prefix . 'pdf_file_path', 'deleted');

        $order->add_order_note(
            ($keep === 'skonto')
                ? __('Paid with Skonto. The ordinary QR invoice has been voided.', 'sqrip-swiss-qr-invoice')
                : __('Paid without Skonto. The QR invoice with Skonto has been voided.', 'sqrip-swiss-qr-invoice')
        );
