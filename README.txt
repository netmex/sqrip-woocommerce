...
== Changelog ==
= 1.9.4 : Juli 2026 – Safeguard =
* Adds a non-intrusive admin warning when the sqrip order status is set to "Pending payment" while stock-hold is active (such orders get auto-cancelled by WooCommerce). Existing shop settings are never changed.

= 1.9.3 : Juli 2026 – Fix =
* Orders no longer get stuck in 'pending': the sqrip order status is now set server-side during checkout instead of relying on the confirmation page, which a full-page cache (e.g. NitroPack) or the block checkout can bypass.
